    ...
    if (err)
        ...
        if (filedata.toString() != '') {
            ...
            liscenseNumber.find({ "licenseNumber": num.trim() }, function (err, data) {
                ...
                num = num.replace(/(\r\n|\n|\r)/gm, "");
                if (err)
                    console.log("Error on car entrance: \n" + err);
                else {
                    if (data.length == 0) {
                        ...
                        var carStack = new AuthorizedCar({
                            ...
                            var result = data;
                            if (err)
                                console.log("Error to maintain data of unregistered car: \n" + err);
                            else {
                                console.log("Data saved in carStack of unregistered car: \n" + data);
                                var mailOptions = {
                                    from: "RedZone Security Systems  pkbscs67@gmail.com",
                                    to: "pk_bscs@yahoo.com",
                                    subject: "Alert...Unregistered car detected",
                        ...
                        for (var i = 0; i < data.length; i++) {
                            var carStack = new AuthorizedCar({
                                ...
                                if (err)
                                    console.log("Error to maintain data of registered car: \n" + err);
                                else {
    ...
    AuthorizedCar.find({}, function (err, data) {
        if (err) {
            ...
            AuthorizedCar.find({ "status": "Unregistered" }, function (err, data) {
                if (data.length == 0) {
                    console.log("first phase: No invalid car");
                }
                else {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        liscenseNumber.find({ "licenseNumber": (data[i].plateNumber).trim() }, function (err, data) {
                            if (data.length == 0) {
                                console.log("No invalid car" + data);
                            }
                            else {
                                for (var i = 0; i < data.length; i++) {
                                    AuthorizedCar.update({ "status": "Unregistered" }, { $set: {
                                            "status": "Registered"
                                        }
                                    }, function (err, data) {
                                        if (err)
...
app.get("/enteries", function (req, res) {
    liscenseNumber.find({}, function (err, data) {
        if (err)
            console.log("Error in fetching enteries: \n" + err);
        else {
            if (data.length == 0) {
...
app.get("/refresh", function (req, res) {
    liscenseNumber.find({}, function (err, data) {
        if (err)
            console.log("Error in refreshing enteries: \n" + err);
        else {
            if (data.length == 0) {
